...
def face_recognise(frame):
    ...
    for encoding in encodings:
        ...
        if True in matches:
            ...
            matchedIND = [i for (i, b) in enumerate(matches) if b]
            counts = {}
...
def visualize_detections_video(image, boxes, mask, nomask,maskper,nomaskper):
  count_mask=0
  count_nomask=0
  ...
  for box in boxes:
    ...
    if face_image.shape[0] and face_image.shape[1]:
      ...
      clas = model.predict(np.expand_dims(face_image,axis=0))
      clas = np.argmax(clas,axis=1)
      if clas[0] == 0:
        count_mask=count_mask+1
      elif clas[0] == 1:
        count_nomask= count_nomask+1    
      text = '{}'.format(class_map[clas[0]])    
      ...
      cv2.rectangle(image,start,end,color_map_video[clas[0]],2)
      cv2.putText(image, class_map[clas[0]], start , cv2.FONT_HERSHEY_SIMPLEX, 0.70, color_map_video[clas[0]], 2)
  return image, maskperc, nomaskperc

def social_dist(img,textp1,textp2,textp3,high, safe):
    low_risk_p = 0       
    high_risk_p = 0      
    safe_p = 0
        ...
        if f is not None:
            ...
            while True:
                ...
                if time_elapsed > 1./frame_rate:
                    ...
                    #print(j)
                    img1, finhigh, finsafe = social_dist(image,textp1,textp2,textp3,high, safe)
                    ...
                    if boxes.size == 0:
                        ...
                        img, finmask, finnomask=visualize_detections_video(image, boxes, mask, nomask,maskper,nomaskper)
                        print(finmask)
